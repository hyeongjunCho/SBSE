...
/*
 * end TSP 2.0 approx (local minima)
 */
 ...
 * push vertexes
 */
	vector<unsigned short> P_V;
	vector<unsigned short>::iterator it;

	P_V.push_back(G->e[(num_vertices*(num_vertices-1)/2)-1]->a);
	P_V.push_back(G->e[(num_vertices*(num_vertices-1)/2)-1]->b);

	vector<unsigned short> random_V;
	
	for (i = 0; i < num_vertices; i++)
	{
		if (i == (int)P_V[0] || i == (int)P_V[1]) continue;
		random_V.push_back((unsigned short)i);
	}
	random_shuffle(random_V.begin(),random_V.end());

	for (auto vertex : random_V)
	{
		i = 0;
		int best_index = 0;
		...
		for (auto cur : P_V)
		{
			d = (int)distance(G->v[vertex],G->v[*it]) + (int)distance(G->v[vertex],G->v[cur]);
			if (best_d >= d)
			{
				best_d = d;
				best_index = i;
		...
		P_V.insert(it, vertex);
		cout << "best d" << best_d << endl;
	}
...
//	}
	...
	total_w = 0.0;
	vector<unsigned short>::iterator it2 = P_V.begin();
	vector<unsigned short>::iterator it3 = next(P_V.begin(),1);
	for (i = 0; i < num_vertices - 1; i++)
	{
		total_w += distance(G->v[*it2], G->v[*it3]);
